...
func newTypeRegistry() *typeRegistry {
	return &typeRegistry{
		...
		cbs: &callbacks{
			m: make(map[string]CallbackFactory),
			l: &sync.RWMutex{},
		},
	}
}
...
func (r *typeRegistry) registerCallbackFactory(name string, f CallbackFactory) {
	r.job.l.Lock()
	defer r.job.l.Unlock()

	if _, exists := r.cbs.m[name]; exists {
		grip.Warningf("job named '%s' is already registered. Overwriting existing value.", name)
	}
...
func (r *typeRegistry) getCallbackFactory(name string) (JobFactory, error) {
	r.job.l.RLock()
	defer r.job.l.RUnlock()

	factory, ok := r.cbs.m[name]
	if !ok {
		return nil, errors.Errorf("there is no job type named '%s' registered", name)
	}
